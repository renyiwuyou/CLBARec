...
class SASRecTrainer(Trainer):
        ...
        if train:
            ...
            for i, (rec_batch, aug_batches) in rec_cf_data_iter:
                ...
                if epoch >= self.args.pretrain_epoch:
                    contrast_loss_total = 0.0
                    for aug_batch in aug_batches:
                        aug_loss, augment_output, aug_sequences = self.aug_mix_learning(input_ids, aug_batch,
                                                                                        target_pos, target_neg)
                        sequence_rep = self.get_sequence_rep(sequence_output, input_ids)
                        augment_rep = self.get_sequence_rep(augment_output, input_ids)
                        aug_output = self.model.transformer_encoder(aug_sequences)
                    ...
                    if len(aug_batches) > 0:
                        contrast_loss_avg = contrast_loss_total / len(aug_batches)
                        joint_loss += self.args.contrast_weight * contrast_loss_avg
    ...
    def aug_mix_learning(self, input_ids, aug_inputs, target_pos, target_neg):
        ...
        return aug_loss, augment_output, aug_sequences

    def get_sequence_rep(self, sequence_output, input_ids):
        batch_size = sequence_output.size(0)
        
        non_pad_mask = (input_ids != 0).float()  # [batch_size, max_seq_len]
        last_non_pad_idx = non_pad_mask.sum(dim=1).long() - 1  # [batch_size]
        last_non_pad_idx = torch.clamp(last_non_pad_idx, min=0) 
        sequence_rep = sequence_output[torch.arange(batch_size), last_non_pad_idx, :]
        return sequence_rep

    def nt_xent_loss(self, rep1, rep2, tau=0.07):
        device = rep1.device
        N = rep1.size(0)
        
        reps = torch.cat([rep1, rep2], dim=0)  # [2N, D]
        
        sim_matrix = torch.nn.functional.cosine_similarity(reps.unsqueeze(1), reps.unsqueeze(0), dim=-1)  # [2N, 2N]
        
        sim_matrix = sim_matrix / tau
       
        pos_mask = torch.zeros((2 * N, 2 * N), dtype=torch.bool, device=device)
        pos_mask[:N, N:] = torch.eye(N, device=device, dtype=torch.bool)  
        pos_mask[N:, :N] = torch.eye(N, device=device, dtype=torch.bool)  
      
        neg_mask = ~torch.eye(2 * N, dtype=torch.bool, device=device) & ~pos_mask
     
        pos_logits = sim_matrix[pos_mask].view(2 * N, 1)  # [2N, 1]
        neg_logits = sim_matrix[neg_mask].view(2 * N, -1)  # [2N, 2N-2]
       
        logits = torch.cat([pos_logits, neg_logits], dim=1)  # [2N, 2N-1]
